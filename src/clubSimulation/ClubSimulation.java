...
public class ClubSimulation {
		...
		startB.addActionListener(new ActionListener() {
			...
			pauseB.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					synchronized (paused) {
						if (paused.get()) {
							paused.set(false); // Resume the simulation
							paused.notifyAll();
						} else {
							paused.set(true); // Pause the simulation
						}
					}
				}
			});
	...
	public static void main(String[] args) throws InterruptedException {
		...
		//Andre always has ID = 0
		//Andre is initialized here
		andreLocation = new PeopleLocation(noClubgoers);
		andreLocation.setInRoom(true);
		andreLocation.setArrived();
		GridBlock bar = clubGrid.whichBlock(1, clubGrid.getBar_y() + 1);
		andreLocation.setLocation(bar);//starting at the top left corner of bar area - synchronized block
		System.out.println("Andre is at: " + (andreLocation.getX() -1) + ":" + andreLocation.getY() );
		int speedAndre = (int)(Math.random() * (maxWait-minWait)+minWait);
		
		AndreBarman andre = new AndreBarman(speedAndre, andreLocation, tallys);

        for (int i=0;i<noClubgoers;i++) {//could be cause of issue
		...
		andre.start();
      	for (int i=0;i<noClubgoers;i++) {
			//check to see if user pressed start button, if they havent - wait
			patrons[i].start();
		}
