...
public class ClubGrid {
	...
	private  void initGrid(int []exitBlocks) throws InterruptedException {
		for (int i=0;i<=x;i++) {//change loop control variable from i=0, i<=x
			for (int j=0;j<y;j++) {//not changed
	...
	//Added method to check if Andre is in bar area
	...
	public GridBlock enterClub(PeopleLocation myLocation) throws InterruptedException  {
		...
		synchronized (entrance){//lock added to ensure that entrance door grid block is only accessed one at a time
		while ((counter.overCapacity()) || (entrance.occupied())) {
			entrance.wait();//if club is full or entrance blocked wait
		}
		entrance.notifyAll();
	...
	public synchronized GridBlock move(GridBlock currentBlock,int step_x, int step_y,PeopleLocation myLocation) throws InterruptedException {  //try to move in 
		...
		synchronized (newBlock) {//added in order to ensure no two clubGoers can access the same newBlock - check before you move
		if (!newBlock.get(myLocation.getID())) return currentBlock; //stay where you are
			
		currentBlock.release(); //must release current block
		myLocation.setLocation(newBlock);
		synchronized (entrance) {//deadlock example
		if ((!counter.overCapacity()) && (!entrance.occupied()))//added to ensure people at the entrance enter instead of waiting
		{entrance.notify();}
		}
		return newBlock;
		}
	...
	public synchronized GridBlock moveAndre(GridBlock currentBlock,int step_x, int step_y,PeopleLocation myLocation) throws InterruptedException {
	//method added since Andre is not in Patron Area
		int c_x= currentBlock.getX();
		int c_y= currentBlock.getY();
